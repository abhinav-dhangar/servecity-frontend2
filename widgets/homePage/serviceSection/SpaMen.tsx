"use client";
import { BasicCarousel, type Props } from "@/components/ui/basic-carousel";
import { useGetMenSalon } from "@/hooks/categories/useGetMenSalon";

// Sample data for the carousel
const sampleOffers: Props[] = [
  {
    id: 1,
    imageSrc:
...
// The demo component
export default function SpaMenSection() {
  const { data } = useGetMenSalon();
  return (
    <div className="w-full min-h-auto bg-background flex flex-col items-center justify-center p-4 md:p-10">
      <div className="w-full max-w-6xl">
        <h2 className="text-3xl font-bold mb-6 text-foreground">Men Salon</h2>
        <BasicCarousel items={data} />
      </div>
    </div>
  );
}
